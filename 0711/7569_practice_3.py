...
# m : 상자 가로 칸수, n : 상자 세로 칸 수, h : 쌓아올려지는 상자의 수
...
for i in range(h):
    ...
    for j in range(n):
        ...
        for z in range(m):
            if temp[j][z] == 1: # 익은 토마토 찾아 큐에 집어넣기
                queue.append((i,j,z)) # z,y,x 순서대로 삽입
    arr.append(temp)
...
while queue: # bfs 수행
    ...
    for i in range(6): # 6 방향으로 익히기
        hhh = hh + directions[i][0]
        nnn = nn + directions[i][1]
        mmm = mm + directions[i][2]
        if 0<=hhh<h and 0<=nnn<n and 0<=mmm<m and arr[hhh][nnn][mmm] == 0: #익힐수 있는 토마토라면
